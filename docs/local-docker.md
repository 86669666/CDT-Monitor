...
4. 最终 `scratch` 镜像：非 root `65532:65532`、`VOLUME /data`、`EXPOSE 8080`、入口 `/cdt-monitor serve`、`STOPSIGNAL SIGTERM`，以及镜像内 `HEALTHCHECK`（`/cdt-monitor healthcheck` → `/healthz`）。Compose 另加 `user: "65532:65532"`、`restart: on-failure:3`（本地不自动 unless-stopped）、`no-new-privileges`、`privileged: false`、`cap_drop: ALL`、`pids_limit: 256`、`mem_limit: 512m`、`cpus: 1.0`（本地 `CDT_WORKERS=2`）、只绑定 `127.0.0.1:43210`、只读根文件系统（`/data` 可写，`/tmp` 为 tmpfs）、`stop_grace_period: 15s`、`stop_signal: SIGTERM`，以及 json-file 日志上限 `10m` × 3。
