...
| `Release Binaries` | 仅手动或被 auto-release `workflow_call` | 不再因 `v*.*.*` tag push 自动跑。仍要 `ENABLE_PRODUCTION_PUBLISH`；artifact 保留 7 天；只有 `publish` job 拿 `contents: write`，且 `draft: true` |
...
- 不要把 Release Binaries 的 `draft: true` 改成正式发布，除非有单独的发布授权。
