...
| `Container Images` | 仅手动或被 auto-release `workflow_call` | 不再因 `dev`/tag push 自动跑。默认只做 linux/amd64 load 校验；QEMU/arm64/multi-arch、login、push 在本 fork 均为 `if: false`；没有 `packages: write` |
