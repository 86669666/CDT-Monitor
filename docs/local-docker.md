...
4. 最终 `scratch` 镜像：非 root `65532:65532`、`VOLUME /data`、`EXPOSE 8080`、入口 `/cdt-monitor serve`、`STOPSIGNAL SIGTERM`，以及镜像内 `HEALTHCHECK`（`/cdt-monitor healthcheck` → `/healthz`）。Compose 另加 `no-new-privileges`、`cap_drop: ALL`、`stop_grace_period: 15s`，以及 json-file 日志上限 `10m` × 3。
