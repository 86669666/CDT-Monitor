...
func TestNormalizeClockTimeUses24HourValues(t *testing.T) {
	for input, want := range map[string]string{"00:34": "00:34", "00：34": "00:34"} {
		got, err := normalizeClockTime(input, true)
		if err != nil || got != want {
			t.Errorf("normalizeClockTime(%q) = %q, %v; want %q", input, got, err, want)
		}
	}
	if got, err := normalizeClockTime("24:00", true); err == nil || got != "" {
		t.Fatalf("normalizeClockTime(24:00) = %q, %v; want rejection", got, err)
	}
}
