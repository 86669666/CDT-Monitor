...
续推证据（`2026-09-20T13:42Z` / 2026-09-20 21:42 Asia/Taipei），对象 `86669666/CDT-Monitor`，当时 HEAD `0662ace` 再加 job container 扫描：

- Publish Guard 现禁止 job 级 `container:`（4 空格缩进）。`auto-release.yml` 的 job 名 `container:` 不受影响。继续用 `ubuntu-latest`
- 这仍不是已经发布，也不是 PR#2 可以合进未加开关 `main`

续推证据（`2026-09-20T13:48Z` / 2026-09-20 21:48 Asia/Taipei），对象 `86669666/CDT-Monitor`，当时 HEAD `fc47f95` 再加 Compose network_mode container/service 扫描：

- Publish Guard 现禁止 Compose `network_mode: container:` / `service:`（`host` 本来就禁止）。保持默认 bridge 加 loopback 发布
- 这仍不是已经发布，也不是 PR#2 可以合进未加开关 `main`

续推证据（`2026-09-20T13:55Z` / 2026-09-20 21:55 Asia/Taipei），对象 `86669666/CDT-Monitor`，当时 HEAD `6d008b5` 再加 docker:// action 扫描：
...
续推证据（`2026-09-20T14:05Z` / 2026-09-20 22:05 Asia/Taipei），对象 `86669666/CDT-Monitor`，当时 HEAD `37fac18` 再加 workflow on: 白名单扫描：
...
续推证据（`2026-09-20T14:33Z` / 2026-09-20 22:33 Asia/Taipei），对象 `86669666/CDT-Monitor`，当时 HEAD `7d3f3aa` 再加 Compose uts/userns 扫描：
...
续推证据（`2026-09-20T14:42Z` / 2026-09-20 22:42 Asia/Taipei），对象 `86669666/CDT-Monitor`，当时 HEAD `d15e22b` 再加 Compose cgroup/cgroupns 扫描：
...
续推证据（`2026-09-20T15:21Z` / 2026-09-20 23:21 Asia/Taipei），对象 `86669666/CDT-Monitor`，当时 HEAD `5b8e664` 再加 actions permission 扫描：
...
续推证据（`2026-09-20T23:57Z` / 2026-09-21 07:57 Asia/Taipei），对象 `86669666/CDT-Monitor`，当时 HEAD `12f5053` 再加 attestations permission 扫描：
...
续推证据（`2026-09-22T05:01Z` / 2026-09-22 13:01 Asia/Taipei），对象 `86669666/CDT-Monitor`，当时 HEAD `30cbe51` 再加 security-events permission 扫描：

- Publish Guard 现禁止任何 `security-events:` permission（含 `read`）。`security-events: write` 本来就禁止。不要让 token 上传 SARIF 或改代码扫描告警
- 这仍不是已经发布，也不是 PR#2 可以合进未加开关 `main`

续推证据（`2026-09-22T05:03Z` / 2026-09-22 13:03 Asia/Taipei），对象 `86669666/CDT-Monitor`，当时 HEAD `4c871a6` 再加 Compose device IO 扫描：

- Publish Guard 现禁止 Compose `blkio_weight` 和 `device_read_bps` / `device_write_bps` / `device_read_iops` / `device_write_iops`。`blkio_config` 本来就禁止。保持默认块设备 IO
- 这仍不是已经发布，也不是 PR#2 可以合进未加开关 `main`

续推证据（`2026-09-22T05:06Z` / 2026-09-22 13:06 Asia/Taipei），对象 `86669666/CDT-Monitor`，当时 HEAD `2f496c3` 再加 pull-requests permission 扫描：

- Publish Guard 现禁止任何 `pull-requests:` permission（含 `read`）。`pull-requests: write` 本来就禁止。不要让 token 读或改 PR
- 这仍不是已经发布，也不是 PR#2 可以合进未加开关 `main`

续推证据（`2026-09-22T05:18Z` / 2026-09-22 13:18 Asia/Taipei），对象 `86669666/CDT-Monitor`，当时 HEAD `00c3015` 再加 Dockerfile USER 扫描：
...
续推证据（`2026-09-22T05:41Z` / 2026-09-22 13:41 Asia/Taipei），对象 `86669666/CDT-Monitor`，当时 HEAD `3359219` 再加 models permission 扫描：

- Publish Guard 现禁止任何 `models:` permission（含 `read`）。不要让 token 调用 GitHub Models
- 这仍不是已经发布，也不是 PR#2 可以合进未加开关 `main`

续推证据（`2026-09-22T05:44Z` / 2026-09-22 13:44 Asia/Taipei），对象 `86669666/CDT-Monitor`，当时 HEAD `55dceed` 再加 Compose develop 扫描：
...
续推证据（`2026-09-22T06:30Z` / 2026-09-22 14:30 Asia/Taipei），对象 `86669666/CDT-Monitor`，当时 HEAD `0e8b85c` 再加 checks permission 扫描：

- Publish Guard 现禁止任何 `checks:` permission（含 `read`）。`checks: write` 本来就在额外写权限组里。不要让 token 读或改 GitHub Checks
- 这仍不是已经发布，也不是 PR#2 可以合进未加开关 `main`

续推证据（`2026-09-22T06:45Z` / 2026-09-22 14:45 Asia/Taipei），对象 `86669666/CDT-Monitor`，当时 HEAD `968c93d` 再加 Compose oom_kill_disable 扫描：
