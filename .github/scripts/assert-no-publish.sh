...
scan_checkout_credentials() {
  ...
  for f in .github/workflows/*.yml; do
    ...
    if grep -q 'actions/checkout@' <<<"$body" && grep -Eq '^[[:space:]]+sparse-checkout(-cone-mode)?:' <<<"$body"; then
      bad "$f: checkout sparse-checkout is forbidden; keep a full fetch-depth 1 clone"
    fi
    ...
    if grep -q 'actions/checkout@' <<<"$body" && grep -Eq '^[[:space:]]+ssh-strict:' <<<"$body"; then
      bad "$f: checkout ssh-strict: is forbidden; do not clone over SSH"
    fi
    if grep -q 'actions/checkout@' <<<"$body" && grep -Eq '^[[:space:]]+clean:' <<<"$body"; then
      bad "$f: checkout clean: is forbidden; keep the default clean workspace"
    fi
    ...
    if grep -q 'actions/checkout@' <<<"$body" && grep -Eq '^[[:space:]]+log-level:' <<<"$body"; then
      bad "$f: checkout log-level: is forbidden; keep the action default"
    fi
...
scan_job_limits() {
  ...
  for f in .github/workflows/*.yml; do
    ...
    if grep -Eq '^[[:space:]]+pull-requests:' <<<"$body"; then
      bad "$f: pull-requests permission is forbidden on this fork"
    fi
    ...
    if grep -Eq '^[[:space:]]+models:' <<<"$body"; then
      bad "$f: models permission is forbidden on this fork"
    fi
    if grep -Eq 'deployments:[[:space:]]*write' <<<"$body"; then
      bad "$f: deployments: write is forbidden on this fork"
    fi
    if grep -Eq '^[[:space:]]+deployments:' <<<"$body"; then
      bad "$f: deployments permission is forbidden on this fork"
    fi
    if grep -Eq 'statuses:[[:space:]]*write' <<<"$body"; then
      bad "$f: statuses: write is forbidden on this fork"
    fi
...
scan_compose() {
  ...
  if grep -Eq '^[[:space:]]+(mem_swappiness|memory_swappiness):' <<<"$body"; then
    bad "$f: mem_swappiness is forbidden; keep memswap_limit 512m"
  fi
