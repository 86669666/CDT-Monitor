...
func TestTraffic24HoursAgoAndAroundTime(t *testing.T) {
	...
	if err = st.Setup(ctx, domain.Config{
		AdminPassword: "Strong-Password-42!", TrafficThreshold: 95, ShutdownMode: "KeepCharging", ThresholdAction: "stop_and_notify", APIInterval: 600, Timezone: "Asia/Shanghai",
		Accounts: []domain.Account{{AccessKeyID: "LTAI-traffic-201", AccessKeySecret: "secret", RegionID: "cn-hongkong", InstanceID: "i-201", MaxTraffic: 200, SiteType: "china"}},
	}); err != nil {
		t.Fatal(err)
	}
	accounts, err := st.ListAccounts(ctx)
	...
	_ = st.AddTrafficStats(ctx, accountID, 10.0, t24Ago)
	_ = st.AddTrafficStats(ctx, accountID, 12.5, t12Ago)
	_ = st.AddTrafficStats(ctx, accountID, 15.292, now)

	traffic24, ok := st.Traffic24HoursAgo(ctx, accountID, now)
	if !ok || traffic24 != 10.0 {
		t.Fatalf("expected 10.0 from 24h ago, got %v (ok=%v)", traffic24, ok)
	}

	trafficAround, ok := st.TrafficAroundTime(ctx, accountID, t12Ago)
...
func TestTrafficAroundTimeIgnoresSamplesOutsideWindow(t *testing.T) {
	...
	if err = st.Setup(ctx, domain.Config{
		AdminPassword: "Strong-Password-42!", TrafficThreshold: 95, ShutdownMode: "KeepCharging", ThresholdAction: "stop_and_notify", APIInterval: 600, Timezone: "Asia/Shanghai",
		Accounts: []domain.Account{{AccessKeyID: "LTAI-traffic-202", AccessKeySecret: "secret", RegionID: "cn-hongkong", InstanceID: "i-202", MaxTraffic: 200, SiteType: "china"}},
	}); err != nil {
		t.Fatal(err)
	}
	accounts, err := st.ListAccounts(ctx)
	...
	if err = st.AddTrafficStats(ctx, accountID, 51.98, start.Add(-8*time.Hour)); err != nil {
		t.Fatal(err)
	}
	if err = st.AddTrafficStats(ctx, accountID, 54.44, start.Add(16*time.Hour+10*time.Minute)); err != nil {
		t.Fatal(err)
	}
	if traffic, ok := st.TrafficAroundTime(ctx, accountID, start); ok {
