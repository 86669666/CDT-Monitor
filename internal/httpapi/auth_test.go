...
func TestStoreValidationErrorsStayPublic(t *testing.T) {
	...
	rec = httptest.NewRecorder()
	writeStoreValidationError(rec, "config_failed", "配置保存失败", errors.New("account remark is invalid"))
	if rec.Code != http.StatusBadRequest || !strings.Contains(rec.Body.String(), "account remark is invalid") {
		t.Fatalf("broken remark status = %d body = %s", rec.Code, rec.Body.String())
	}
	rec = httptest.NewRecorder()
	...
	rec = httptest.NewRecorder()
	writeStoreValidationError(rec, "config_failed", "配置保存失败", errors.New("notification port is invalid"))
	...
	rec = httptest.NewRecorder()
	writeStoreValidationError(rec, "config_failed", "配置保存失败", errors.New("account max traffic is invalid"))
...
func TestSaveConfigRejectsInvalidScheduleClockHTTP(t *testing.T) {
	st := initializedAuthStore(t)
	handler := testAPIHandler(t, st)
	session, csrf := loginCookies(t, handler)
	cookies := []*http.Cookie{session, csrf}
	...
	config.Accounts[0].StartTime = "24:00"
	config.Accounts[0].StopTime = "08:00"
	raw, err := json.Marshal(config)
	if err != nil {
		t.Fatal(err)
	}
...
func TestSaveConfigRejectsMetadataSMTPHost(t *testing.T) {
	st := initializedAuthStore(t)
	handler := testAPIHandler(t, st)
	session, csrf := loginCookies(t, handler)
	got := doRequest(t, handler, http.MethodGet, "/api/v1/config", "", []*http.Cookie{session, csrf}, nil)
	...
	config.Notifications.Email.Host = "169.254.169.254"
	raw, err := json.Marshal(config)
	if err != nil {
		t.Fatal(err)
	}
	bad := doRequest(t, handler, http.MethodPut, "/api/v1/config", string(raw), []*http.Cookie{session, csrf}, map[string]string{"X-CDT-CSRF": csrf.Value})
	if bad.Code != http.StatusBadRequest || !strings.Contains(bad.Body.String(), "config_failed") {
		t.Fatalf("metadata smtp host status = %d body = %s", bad.Code, bad.Body.String())
	}
}
