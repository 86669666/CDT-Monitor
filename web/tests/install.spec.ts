...
test('boot status unauthorized opens login', async ({ page }) => {
  await mockInitStatus(page, true)
  await page.route('**/api/v1/status', (route) => route.fulfill({
    status: 401,
    json: { error: { code: 'unauthorized', message: '请登录或提供有效 API Key' } },
...
test('about update check surfaces the live unauthorized envelope', async ({ page }) => {
  let checkCalls = 0
  await mockInitStatus(page, true)
  await mockDashboardReads(page)
  await page.route('**/api/v1/system/info**', (route) => {
...
test('about update check surfaces the live not_found envelope', async ({ page }) => {
  let checkCalls = 0
  await mockInitStatus(page, true)
  await mockDashboardReads(page)
  await page.route('**/api/v1/system/info**', (route) => {
...
test('instance start refresh opens login on live unauthorized status', async ({ page }) => {
  ...
  await page.route('**/api/v1/accounts/1/actions/start', (route) => {
    expect(route.request().method()).toBe('POST')
    return route.fulfill({ status: 202, json: jobFixture('start-ok', 'queued', 1) })
  })
  await page.route('**/api/v1/jobs/**', (route) => {
    failStatus = true
    return route.fulfill({ json: jobFixture('start-ok', 'completed', 1) })
  })
...
test('instance stop refresh opens login on live unauthorized status', async ({ page }) => {
  let failStatus = false
  let statusCalls = 0
  await mockInitStatus(page, true)
  await mockDashboardReads(page)
  ...
  await page.route('**/api/v1/accounts/1/actions/stop', (route) => {
    expect(route.request().method()).toBe('POST')
    return route.fulfill({ status: 202, json: jobFixture('stop-ok', 'queued', 1) })
  })
  await page.route('**/api/v1/jobs/**', (route) => {
    failStatus = true
    return route.fulfill({ json: jobFixture('stop-ok', 'completed', 1) })
  })
...
test('refresh-all opens login on live unauthorized status', async ({ page }) => {
  let failStatus = false
  let statusCalls = 0
  await mockInitStatus(page, true)
  await mockDashboardReads(page)
  ...
  await page.route('**/api/v1/accounts/refresh', (route) => {
    expect(route.request().method()).toBe('POST')
    return route.fulfill({ status: 202, json: { jobs: [jobFixture('refresh-all-ok', 'queued')] } })
  })
  await page.route('**/api/v1/jobs/**', (route) => {
    failStatus = true
    return route.fulfill({ json: jobFixture('refresh-all-ok', 'completed') })
  })
...
test('refresh-all keeps the console on live status_failed', async ({ page }) => {
  let failStatus = false
  let statusCalls = 0
  await mockInitStatus(page, true)
  await mockDashboardReads(page)
  ...
  await page.route('**/api/v1/accounts/refresh', (route) => {
    expect(route.request().method()).toBe('POST')
    return route.fulfill({ status: 202, json: { jobs: [jobFixture('refresh-all-status', 'queued')] } })
  })
  await page.route('**/api/v1/jobs/**', (route) => {
    failStatus = true
    return route.fulfill({ json: jobFixture('refresh-all-status', 'completed') })
  })
...
test('instance start keeps the console on live status_failed', async ({ page }) => {
  let failStatus = false
  let statusCalls = 0
  const stopped = {
    ...dashboardStatus,
  ...
  await page.route('**/api/v1/accounts/1/actions/start', (route) => {
    expect(route.request().method()).toBe('POST')
    return route.fulfill({ status: 202, json: jobFixture('start-status', 'queued', 1) })
  })
  await page.route('**/api/v1/jobs/**', (route) => {
    failStatus = true
    return route.fulfill({ json: jobFixture('start-status', 'completed', 1) })
  })
...
test('instance stop keeps the console on live status_failed', async ({ page }) => {
  let failStatus = false
  let statusCalls = 0
  await mockInitStatus(page, true)
  await mockDashboardReads(page)
  ...
  await page.route('**/api/v1/accounts/1/actions/stop', (route) => {
    expect(route.request().method()).toBe('POST')
    return route.fulfill({ status: 202, json: jobFixture('stop-status', 'queued', 1) })
  })
  await page.route('**/api/v1/jobs/**', (route) => {
    failStatus = true
    return route.fulfill({ json: jobFixture('stop-status', 'completed', 1) })
  })
...
test('instance refresh keeps the console on live status_failed', async ({ page }) => {
  let failStatus = false
  let statusCalls = 0
  await mockInitStatus(page, true)
  await mockDashboardReads(page)
  ...
  await page.route('**/api/v1/accounts/1/refresh', (route) => {
    expect(route.request().method()).toBe('POST')
    return route.fulfill({ status: 202, json: jobFixture('refresh-status', 'queued', 1) })
  })
  await page.route('**/api/v1/jobs/**', (route) => {
    failStatus = true
    return route.fulfill({ json: jobFixture('refresh-status', 'completed', 1) })
  })
...
test('instance refresh keeps the console on live forbidden', async ({ page }) => {
  let failStatus = false
  let statusCalls = 0
  await mockInitStatus(page, true)
  await mockDashboardReads(page)
  ...
  await page.route('**/api/v1/jobs/**', (route) => {
    failStatus = true
    return route.fulfill({ json: jobFixture('refresh-forbidden', 'completed', 1) })
  })
...
test('instance refresh keeps the console on live not_found', async ({ page }) => {
  let failStatus = false
  let statusCalls = 0
  await mockInitStatus(page, true)
  await mockDashboardReads(page)
  ...
  await page.route('**/api/v1/accounts/1/refresh', (route) => {
    expect(route.request().method()).toBe('POST')
    return route.fulfill({ status: 202, json: jobFixture('refresh-missing', 'queued', 1) })
  })
  await page.route('**/api/v1/jobs/**', (route) => {
    failStatus = true
    return route.fulfill({ json: jobFixture('refresh-missing', 'completed', 1) })
  })
...
test('instance refresh keeps the console on live internal_error', async ({ page }) => {
  let failStatus = false
  let statusCalls = 0
  await mockInitStatus(page, true)
  await mockDashboardReads(page)
  ...
  await page.route('**/api/v1/jobs/**', (route) => {
    failStatus = true
    return route.fulfill({ json: jobFixture('refresh-internal', 'completed', 1) })
  })
...
test('instance start keeps the console on live forbidden', async ({ page }) => {
  let failStatus = false
  let statusCalls = 0
  const stopped = {
    ...dashboardStatus,
  ...
  await page.route('**/api/v1/jobs/**', (route) => {
    failStatus = true
    return route.fulfill({ json: jobFixture('start-forbidden', 'completed', 1) })
  })
...
test('instance start keeps the console on live not_found', async ({ page }) => {
  let failStatus = false
  let statusCalls = 0
  const stopped = {
    ...dashboardStatus,
  ...
  await page.route('**/api/v1/jobs/**', (route) => {
    failStatus = true
    return route.fulfill({ json: jobFixture('start-missing', 'completed', 1) })
  })
...
test('instance stop keeps the console on live not_found', async ({ page }) => {
  let failStatus = false
  let statusCalls = 0
  await mockInitStatus(page, true)
  await mockDashboardReads(page)
  ...
  await page.route('**/api/v1/jobs/**', (route) => {
    failStatus = true
    return route.fulfill({ json: jobFixture('stop-missing', 'completed', 1) })
  })
...
test('instance stop keeps the console on live internal_error', async ({ page }) => {
  let failStatus = false
  let statusCalls = 0
  await mockInitStatus(page, true)
  await mockDashboardReads(page)
  ...
  await page.route('**/api/v1/jobs/**', (route) => {
    failStatus = true
    return route.fulfill({ json: jobFixture('stop-internal', 'completed', 1) })
  })
...
test('refresh-all keeps the console on live forbidden', async ({ page }) => {
  let failStatus = false
  let statusCalls = 0
  await mockInitStatus(page, true)
  await mockDashboardReads(page)
  ...
  await page.route('**/api/v1/accounts/refresh', (route) => {
    expect(route.request().method()).toBe('POST')
    return route.fulfill({ status: 202, json: { jobs: [jobFixture('refresh-all-forbidden', 'queued')] } })
  })
  await page.route('**/api/v1/jobs/**', (route) => {
    failStatus = true
    return route.fulfill({ json: jobFixture('refresh-all-forbidden', 'completed') })
  })
...
test('refresh-all keeps the console on live not_found', async ({ page }) => {
  let failStatus = false
  let statusCalls = 0
  await mockInitStatus(page, true)
  await mockDashboardReads(page)
  ...
  await page.route('**/api/v1/accounts/refresh', (route) => {
    expect(route.request().method()).toBe('POST')
    return route.fulfill({ status: 202, json: { jobs: [jobFixture('refresh-all-missing', 'queued')] } })
  })
  await page.route('**/api/v1/jobs/**', (route) => {
    failStatus = true
    return route.fulfill({ json: jobFixture('refresh-all-missing', 'completed') })
  })
