...
检查时间：2026-09-09 02:19 Asia/Taipei（`2026-09-08T18:19Z`）。这台 ops 工作区 **不能** 本地出包，不要把本机未构建写成 APK 已验证。`java` / `javac` 仍不存在，`JAVA_HOME` 为空。
